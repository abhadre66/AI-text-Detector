...
def evaluate_on_test(model, X_test, y_test, scaler):
    ...
    metrics = evaluate_model(y_test, y_pred, y_proba, "Logistic Regression (Test)")
...
def plot_confusion_matrix(y_true, y_pred, save_dir="models"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "logistic_regression_confusion_matrix.png"), dpi=150)
    print(f"Saved confusion matrix to {save_dir}/logistic_regression_confusion_matrix.png")
    plt.close()
...
def plot_roc_curve(y_true, y_proba, save_dir="models"):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.plot(fpr, tpr, color="blue", lw=2, label=f"Logistic Regression (AUC = {auc:.4f})")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "logistic_regression_roc_curve.png"), dpi=150)
    print(f"Saved ROC curve to {save_dir}/logistic_regression_roc_curve.png")
    plt.close()
...
def plot_feature_importance(model, feature_names, save_dir="models", top_n=15):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "logistic_regression_feature_importance.png"), dpi=150)
    print(f"Saved feature importance chart to {save_dir}/logistic_regression_feature_importance.png")
    plt.close()
...
def save_model(model, scaler, metrics, feature_names, save_dir="models"):
    """Save the Logistic Regression model, scaler, and metadata."""
    os.makedirs(save_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    lr_model, lr_scaler, val_metrics = train_logistic_regression(X_train, y_train, X_val, y_val)

    # Final evaluation on test set
    test_metrics, test_preds, test_proba = evaluate_on_test(lr_model, X_test, y_test, lr_scaler)
    ...
    # Save model
    save_model(lr_model, lr_scaler, test_metrics, feature_names)
