...
def train_xgboost(X_train, y_train, X_val, y_val):
    """
    Train XGBoost classifier with grid search over hyperparameters.
...
def evaluate_on_test(model, X_test, y_test):
    ...
    metrics = evaluate_model(y_test, y_pred, y_proba, "XGBoost (Test)")
...
def plot_confusion_matrix(y_true, y_pred, save_dir="models"):
    ...
    print(f"Saved confusion matrix to {save_dir}/xgboost_confusion_matrix.png")
    plt.close()
...
def plot_roc_curve(y_true, y_proba, save_dir="models"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "xgboost_roc_curve.png"), dpi=150)
    print(f"Saved ROC curve to {save_dir}/xgboost_roc_curve.png")
    plt.close()
...
def plot_feature_importance(model, feature_names, save_dir="models", top_n=15):
    ...
    print(f"Saved feature importance chart to {save_dir}/xgboost_feature_importance.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Train XGBoost
    xgb_model, val_metrics = train_xgboost(X_train, y_train, X_val, y_val)

    # Final evaluation on test set
    test_metrics, test_preds, test_proba = evaluate_on_test(xgb_model, X_test, y_test)
    ...
    # Save model
    save_model(xgb_model, test_metrics, feature_names)
